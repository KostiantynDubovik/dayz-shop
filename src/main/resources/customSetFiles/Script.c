...
class CustomSpawnPlayerConfig
{
	...
	ref map<string, ref array<string>> players_sets = new map<string, ref array<string> >;
	...
	void LoadFileConfig(array<string> readed_lines_config)
	{
		...
		foreach (string line : readed_lines_config)
		{
			...
			array<string> temp_players_sets = new array<string>;
			temp_players_sets.Clear();
			array<string> temp_players_spawnpoints_string = new array<string>; temp_players_spawnpoints_string.Clear();
			array<string> temp_players_sets_string = new array<string>;
			temp_players_sets_string.Clear();
			...
			foreach (string line_set : temp_players_sets_string)
			{
				...
				{
					temp_players_sets.Insert(line_set);
				}
			}
	...
	void Load_And_Check_StartLoadout(PlayerBase player)
	{
		...
		if(identity)
		{
			...
			if (enabled_loading_custom_sets)
			{
				if ((players_sets.Count() > 0) && (enabled_loaded_successfull_config))
				{
					if (players_sets.Contains(UID_P))
					{
						...
						private string chek_use_set_id = readed_lines_config.GetRandomElement();
						...
						} else
						{
							...
							// Print(Log_CustomSpawnPlayerConfig_GetDateTime() + default_log_block_name + name_block_work + "For player " + Name_P + "(steam64id=" + UID_P + ") setuped private set number is: " + use_set_id);
						}
					} else
					{
			...
			if (use_set_id != "-1")
			{
				// Print(Log_CustomSpawnPlayerConfig_GetDateTime() + default_log_block_name + name_block_work + "For player " + Name_P + "(steam64id=" + UID_P + ") will be used spawn set with number is: " + use_set_id);
			} else
			{
